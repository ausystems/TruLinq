...
* **The luxury layer (2026-09-22).** Three scroll-linked behaviours give the site its Apple-like calm, and every page
  gets them from `boot()`: (1) every panel settles as it enters (`initPanels`: scale .975 and 55% presence at the bottom
  of the screen, full by the middle, scrubbed; the hero and pinned sections are exempt); (2) on desktop the homepage
  hero recedes and fades as the next panel comes up over it; (3) `data-reveal` entrances settle from a soft blur on
  pointer devices (never on phones). Buttons carry a faint top light. No cursor-following effects anywhere: the user
  removed the card spotlight on purpose; do not bring it back.
